...
class Image {
     ...
     *
     * @static public
	* @param array $arr 水印信息 具体看下面的例子
    * @return void
	 *参数说明 注意数组下标不能改变
		图片水印例子
		$arr = array();
		$arr["source"] = '1.jpg'; //被打水印的图片
		$arr["savename"] = null; //添加水印后的图片名
		$arr["watermarktype"] = WATER_MARK_TYPE;//水印类型（图片1或文字0）
		$arr["watermarkwidth"] = WATER_MARK_WIDTH;//被打水印的图片的最小宽度 少于该尺寸则不加水印
		$arr["watermarkheight"] = WATER_MARK_HEIGHT;//被打水印的图片的最小高度 少于该尺寸则不加水印
		$arr["watermarkimg"] = WATER_MARK_IMG;//水印图片
		$arr["watermarkpct"] = WATER_MARK_PCT; //水印透明度
		$arr["watermarkquality"] = WATER_MARK_QUALITY; //JPEG水印图像质量
		$arr["watermarkpos"] = WATER_MARK_POS;//水印位置 默认右下角
		文字水印例子
		$arr = array();
		$arr["source"] = '1.jpg'; //被打水印的图片
		$arr["savename"] = null; //添加水印后的图片名
		...
		$arr["watermarkwidth"] = WATER_MARK_WIDTH;//被打水印的图片的最小宽度 少于该尺寸则不加水印
		$arr["watermarkheight"] = WATER_MARK_HEIGHT;//被打水印的图片的最小高度 少于该尺寸则不加水印
		$arr["watermarktext"] = WATER_MARK_TEXT;//水印文字
		$arr["watermarkfontsize"] = WATER_MARK_FONT_SIZE;//水印文字大小
		$arr["watermarkfontcolor"] = WATER_MARK_FONT_COLOR;//水印文字颜色
		$arr["watermarkpct"] = WATER_MARK_PCT; //水印透明度
		$arr["watermarkquality"] = WATER_MARK_QUALITY; //JPEG水印图像质量
		$arr["watermarkpos"] = WATER_MARK_POS;//水印位置 默认右下角
     */
    static public function water($arr) {
		//file_put_contents('log.txt',var_export($arr,true));
        // 检查被水印的文件是否存在
        if (!file_exists($arr['source']))
            return false;
		// 获取被使用图片信息
        $sInfo = self :: getImageInfo($arr['source']);
		//如果图片不符合水印条件，不生成图片
        if ($sInfo["width"] < $arr["watermarkwidth"] || $sInfo['height'] < $arr["watermarkheight"])
            return false;
		// 建立图像
	    $sCreateFun = "imagecreatefrom" . $sInfo['type'];
		$sImage = $sCreateFun($arr['source']);
		// 水印类型是图片
		if($arr['watermarktype'] == '1') {
			//处理一下水印图片路径
			//$arr['watermarkimg'] = preg_replace('~/upload~','.',$arr['watermarkimg']);
            $arr['watermarkimg']=ROOT_PATH.$arr['watermarkimg'];
			//print_r($arr);
            //var_dump(file_exists($arr['watermarkimg']));
			//检查水印图片是否存在
			if (!file_exists($arr['watermarkimg']))
				return false;
			//获取水印图片信息
			$wInfo = self :: getImageInfo($arr['watermarkimg']);
			// 如果图片小于水印图片，不生成图片
			if ($sInfo["width"] < ($wInfo["width"]+15) || $sInfo['height'] < ($wInfo['height']+15))
				return false;
			// 建立图像
			$wCreateFun = "imagecreatefrom" . $wInfo['type'];
			$wImage = $wCreateFun($arr['watermarkimg']);
			// 设定图像的混色模式
			imagealphablending($wImage, true);
			// 计算水印位置
			switch($arr["watermarkpos"]){
						//左上角
						case 1:
							$posX = 15;
							$posY = 15;
						...
						//右上角
						case 2:
							$posX = $sInfo["width"] - $wInfo["width"] - 15;
							$posY = 15;
							break;
						//左下角
						case 4:
							$posX = 15;
							$posY= $sInfo["height"] - $wInfo["height"] - 15;
							break;
						//居中
						case 5:
							$posX = round($sInfo["width"]/2 - $wInfo["width"]/2);
							$posY = round($sInfo["height"]/2 - $wInfo["height"]/2);
							break;
						//随机
						case 0:
			...
			// 生成混合图像
			$o=imagecopymerge($sImage, $wImage, $posX, $posY, 0, 0, $wInfo['width'], $wInfo['height'], $arr['watermarkpct']);
            //var_dump($o);
		}
		// 水印类型是文字
		if($arr['watermarktype'] == '0'){
			//计算十六进制颜色
			if(strlen($arr["watermarkfontcolor"]) != 7) {
				$r = '0xD2';
				$g = '0xC0';
				$b = '0xA5';
			}else{
				$r ='0x'.substr($arr["watermarkfontcolor"],1,2);
				$g ='0x'.substr($arr["watermarkfontcolor"],3,2);
				$b ='0x'.substr($arr["watermarkfontcolor"],5,2);
			}
			// 创建背景色
			$bg = imagecolorallocate($sImage, $r, $g, $b);
			// 计算水印文本的大小
			$box = imagettfbbox($arr['watermarkfontsize'],0,$arr['water_mark_font_family'],$arr['watermarktext']);
			$box_width =  $box[2] - $box[6];
			$box_height = $box[3] - $box[7];
			//如果图片小于水印文本
			if ($sInfo["width"] < ($box_width + 15) || $sInfo['height'] < ($box_height + 15))
				return false;
			// 计算水印位置
			switch($arr["watermarkpos"]){
						//左上角
						...
						//右上角
						case 2:
							$posX = $sInfo["width"] - $box_width -15;
							$posY = $box_height + 15;
							break;
						//左下角
						case 4:
							$posX = 15;
							$posY= $sInfo["height"] - 15;
							break;
						//居中
						case 5:
							$posX = round($sInfo["width"]/2 - $box_width/2);
							$posY = round($sInfo["height"]/2 - $box_height/2);
							break;
						//随机
						case 0:
							$posX = rand(0,($sInfo["width"]-$box_width));
							$posY = rand($box_height,$sInfo["height"]);
							break;
						//右下角
						case 3:
						default:
			...
			//将文字打印到图像上
			imagettftext($sImage,$arr['watermarkfontsize'],0,$posX,$posY,$bg,$arr['water_mark_font_family'],$arr['watermarktext']);
		}
        // 输出图像
		...
		if ($sInfo['type'] == "jpg" || $sInfo['type'] == "jpeg") {
			imagejpeg($sImage, $savename, $arr["watermarkquality"]);//第3个参数即使质量大小，因为只有imagejpeg支持这个参数
		} else {
    ...
    static function thumb($image, $thumbname, $type = '', $maxWidth = 200, $maxHeight = 50, $interlace = true) {
        ...
        if ($info !== false) {
            ...
            if ('png' == $type) {
                ...
                if ($trnprt_indx >= 0 && $trnprt_indx < 256) {
                    // its transparent
