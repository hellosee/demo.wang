...
class UploadFile { // 类定义开始
    ...
    public function __construct($config = array()) {
		...
		//file_put_contents('log.txt',var_export($this->water,true));
    }
    ...
    private function save($file) {
		//file_put_contents('log.txt',var_export($file,true));
        ...
        $source_file_name=$this -> autoCharset($filename, 'utf-8', 'gbk');
        if (!move_uploaded_file($file['tmp_name'], $source_file_name)) {
            $this -> error = '文件上传保存错误！';
            return false;
        }
        // 如果是图像文件 检测文件格式，兼容某些任何图片都无法识别上传问题
        if (in_array(strtolower($file['extension']), array('gif', 'jpg', 'jpeg', 'bmp', 'png', 'swf'))) {
            $info = getimagesize($source_file_name);
        ...
        */
		...
		if($this->config['water']){
			//将图片路径保存到water中
			$this->config['water']['source'] = $this -> autoCharset($filename, 'utf-8', 'gbk');
			//file_put_contents('log.txt',var_export($this->config['water'],true));
			Image :: water($this->config['water']);
			//将图片缩略图路径保存到water中
			$this->config['water']['source'] = $thumbPath . $thumbname . '.' . $thumbExt;
			Image :: water($this->config['water']);
            //file_put_contents('log.txt',var_export($this->config['water'],true));
		}
        if ($this -> zipImags) {
    ...
    public function upload($savePath = '') {
		...
		//file_put_contents('log.txt',var_export($files,true));
    ...
    private function check($file) {
        ...
        if (!$this -> checkSize($file['size'])) {
            if(in_array($file['extension'],array('jpg','jpeg','bmp','gif','png'))) {
                $this->error = '图片最大允许：' . ($this->maxSize / 1024) . 'KB<br>请用 Photoshop 等图片软件处理后上传';
                return false;
            }
            $this -> error = '上传文件大小不符！最大允许：'.($this -> maxSize/1024).'KB';
            return false;
        }
        // 检查文件Mime类型
